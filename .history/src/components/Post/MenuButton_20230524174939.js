...
const MenuButton = () => {
  ...
  const onShowOwnedPostHandler = async() =>
  {
    try
    {
      let response = await PostService.getPostsByUserId(currentUser._id);
      setFilterPosts(response.data);
    }
    catch (err)
    {
  ...
  return (
    <Fragment>
      <div className={styles.menuButtons}>
        <button className={styles.menuButton} onClick={getPosts}>
        ...
        <button className={styles.menuButton} onClick={onHotPostHandler}>&#128293; HOT</button>
        {currentUser && <button className={styles.menuButton} onClick={onShowSavedPostHandler}>&#128190; SAVED</button>}
        {currentUser && <button className={styles.menuButton} onClick={onShowOwnedPostHandler}>&#128213; OWNED</button>}
        {currentUser && <button className={styles.newPost} onClick={displayCreatePost}>&#10133;</button>}
      </div>
      {createPostIsShown && (
