...
const PostReply = (props) => {
  ...
  const onReplyDislikeHandler = async () =>
  {
    try
    {
      await PostService.dislikeReply(currentPost._id, props.index);
      getPosts();
      refreshCurrentPost();
    }
    catch (err)
      ...
      <div className={styles.reply}>
        <div className={styles.info}>
          ...
          <div className={styles.detailsInfo}>
            ...
            <p>
              {props.reply.dislike.length} <button onClick={onReplyDislikeHandler}>&#128078;</button>
            </p>
            <p className={styles.date}>{shortDate}</p>
          </div>
        </div>
        <div className={styles.content}>
          <p dangerouslySetInnerHTML={{ __html: props.reply.content.replace(/\n/g, '<br>') }}></p>
