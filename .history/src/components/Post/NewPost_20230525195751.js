...
const NewPost = (props) => {
  ...
  useEffect(() => {
    if (title.length == 0) {
      ...
      setTitleWarningMessage(
        "The character of title should be between 3 to 100"
      );
    } else {
      setTitleWarningMessage("");
    }
  ...
  const onCreatePost = async() =>
  {
    try
    {
        ...
        // get the post user created and set it to current post
        let newPost = await PostService.getPostById(_id);
    ...
    {
      console.log(err);
      setContentWarningMessage(err.message);
    }
  }
  ...
  return (
    <Modal>
      <div className={styles.popup}>
        ...
        {titleWarningMessage && <p className={styles.warningMessage}>{titleWarningMessage}</p>}
        ...
        {contentWarningMessage && <p className={styles.warningMessage}>{contentWarningMessage}</p>}
